...
class GCCTimeSeries(object):
    ...
    def readCSV(self, gccTimeSeriesPath):
        ...
        for row in csvrdr:
            ...
            if row["image_count"] == 0:
                ...
                row["outlierflag_gcc_mean"] = _int_or_none(row["outlierflag_gcc_mean"])
                row["outlierflag_gcc_50"] = _int_or_none(row["outlierflag_gcc_50"])
                row["outlierflag_gcc_75"] = _int_or_none(row["outlierflag_gcc_75"])
                row["outlierflag_gcc_90"] = _int_or_none(row["outlierflag_gcc_90"])
    ...
    def format_csvrow(self, gccts_row):
        ...
        elif (gccts_row["image_count"] < self.nmin) and (gccts_row["image_count"] > 0):
            line_format = (
                "{0},{1},{2},{3},{4},"
                + "{5:.5f},{6:.5f},{7:.5f},{8:.5f},{9:.5f},"
                + "{10},{11},{12},{13},{14},"
    ...
    def writeCSV(self, file=""):
        ...
        hdstrings.append("#\n")
        hdstrings.append(
            ("# {0}-day summary product time" + "series for {1}\n").format(
                self.nday, self.site
            )
        )
        hdstrings.append("#\n")
        ...
        for row in self.rows:
            ...
            if row["image_count"] >= self.nmin:
                ...
                formatted_row["midday_gcc"] = "{0:.5f}".format(row["midday_gcc"])
                formatted_row["midday_rcc"] = "{0:.5f}".format(row["midday_rcc"])
